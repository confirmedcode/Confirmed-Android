...
public class CharonVpnService extends VpnService implements Runnable, VpnStateService.VpnStateListener
{
		...
		if (intent != null)
		{
			...
			else
			{
				...
				if (bundle != null)
				{
					...
					isToggle = bundle.getBoolean("VPN_IS_TOGGLE", false);
					if (profile != null)
					{
						if(!isToggle || mService == null) {
							String password = bundle.getString(VpnProfileDataSource.KEY_PASSWORD);
							profile.setPassword(password);
						}
						else
						{
							State connectionStation = mService.getState();
							if(connectionStation == State.CONNECTED)
							{
								profile = null; // disconnect
							}
						}
					}
				}
	...
	public class BuilderAdapter
	{
		...
		private VpnService.Builder createBuilder(String name) {
			...
			// init in case started through widget
			try {
				PreferenceStoreUtil.getInstance().initializeStore(StrongSwanApplication.getContext());
			} catch (Exception e) {
				Log.e(TAG, "Error initializing secured pref store: " + e.getMessage());
			}

			Set<String> excludeList = PreferenceStoreUtil.getInstance().getStringSet("application_exclude_list", null);
	...
	public class BuilderCache
	{
		...
		public void applyData(VpnService.Builder builder)
		{
			...
			/* same thing for IPv6 */
			if ((mSplitTunneling & VpnProfile.SPLIT_TUNNELING_BLOCK_IPV6) == 0 && 1==0) // for now block all ipv6
			{
				if (mIPv6Seen)
				{
					IPRangeSet ranges = new IPRangeSet();
